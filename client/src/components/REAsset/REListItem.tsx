...
export function REListItem({ REInfo, onClick, disabled, className, handleOpenAddREAsset, handleOpenPictures }: any) {
  return (
    <div className={(className ? className + " " : "") + (disabled ? "" : "hover:scale-105 cursor-pointer ") + "relative flex lg:p-5 rounded-xl m-2 lg:m-5 h-28 max-w-screen lg:h-auto lg:min-h-36 bg-zinc-900 shadow-2xl"}
      onClick={onClick}
    >
      {disabled && // Show edit button when disabled
        <button onClick={handleOpenAddREAsset} className=""><span className="material-icons p-2 absolute right-0 top-0">edit</span></button>
      }
      <button onClick={handleOpenPictures} className="lg:rounded-xl rounded-l-xl w-28 h-full lg:w-28 lg:h-28">
